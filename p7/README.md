# CS537 P7
> UW-Madison COMP SCI 537 (CS537) Operating System Project 7: FUSE file system.  
> Originally from Harvey Mudd College (HMC). [See their writeup](https://www.cs.hmc.edu/~geoff/classes/hmc.cs135.201001/homework/fuse/fuse_doc.html).  
> Time taken: 23h without fsck, 38h with fsck (and not finished). 
  
1. In this project, the most important thing to do is to create a **"get inode from path"** helper function. Making sure this specific helper function works correctly guarantees the correctness of the operations later.  
2. We failed to create the fsck.wfs.c file, which counts as a bonus. But the basic idea is to change the operations in the "mount.wfs.c", in order to save every memory operations to the disk file, thus making sure the changes made by fsck can be reflected to the file system. Check the [msync(2)](https://man7.org/linux/man-pages/man2/msync.2.html) Linux page.  
